...
| 用途 | 準備中 | 終了後 |
|---|---|---|
| 修論 | use（版を重ねながら） | git pushしたらそのまま |
| 学会・academic application | use（版を重ねながら） | freeze で固定 |
| グループミーティング | use（版を重ねながら） | freeze で固定 |

**終了後のfreeze：**
```bash
python3 ~/Desktop/figure-hub/scripts/figure_hub.py freeze \
  --project groupmeeting_2026-03-05 \
  --snapshot groupmeeting_2026-03-05_done
```
freezeしたプロジェクトのロックは変更不可になり、「あのとき何の版を使ったか」が永久に残る。

### 図修正の手順（register → sync → git）

**ユーザーが「言わなくていい」ように、以下のトリガーでCursorが自動的にワークフローを開始する。**

**自動検出トリガー：**
- ファイルパス（`.pdf` / `.png` / `.svg` / `.afdesign`）が会話に登場した
- 「直した」「修正した」「できた」「書き出した」「export」などの言葉 + 図に関する文脈

トリガーを検出したら、ユーザーに確認せずに以下を順番に実行する。

**Step 1. register（libraryに新バージョン登録）**
```bash
python3 ~/Desktop/figure-hub/scripts/figure_hub.py register \
  --id <fig_id> \
  --src <書き出したファイルのパス> \
  --note "<修正内容>"
```

**Step 2. use（修論プロジェクトのロックを更新）**
```bash
python3 ~/Desktop/figure-hub/scripts/figure_hub.py use \
  --project thesis_overleaf \
  --id <fig_id> \
  --version latest \
  --dest "<thesis/figure/内のファイル名>"
```

**Step 3. sync（修論ディレクトリに実ファイルをコピー）**
```bash
...
**Step 5. Obsidian更新（figure_fix_inbox.mdのstatusをfixedに）**
該当する `fig_id` のエントリを `status: fixed`・`note: vXXX register済み（日付）` に更新する。

**Step 6. 完了報告**
「fig_xxx v002 を登録・修論に反映・Overleafにpushしました」と1行で報告する。
...
- 作業ログ形式の正本は `/Users/kitak/QPI_Omni/docs/WORKLOG_SPEC.md` とする。
