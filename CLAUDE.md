# QPI_Omni のルール（このリポジトリでだけ効く）

全体のルール（Notion GTD の運用・固定スケジュール・文章の書き方）は `~/.claude/CLAUDE.md`
（dotfiles の `claude/CLAUDE.md`）にある。ここには**このリポジトリと装置でしか成り立たない話**だけを書く。
同じことを両方に書かない。ずれたら全体側が正。

このファイルが正本で、`AGENTS.md` は中身を持たない（Codex 用のポインタ1行だけ）。

---

# 待ちが出たら Notion にタスクを2件作る

顕微鏡PC・解析用パソコンでの決まり。**Mac 側の秘書は、装置がいつ空くかを知らない。**
2026-09-09 に grid 撮影が 24:00 まで回っていたのに、Mac 側は 22:00 に細胞ローディングを
置いていて、その日の実験が丸ごと流れた。装置の占有を Notion に出すのはこれを防ぐため。

## いつ書くか

- 本人が「〜を始めました」と言ったとき
- background コマンドの `<task-notification>` が返ってきたとき
- 所要が 30 分を超える待ちが発生したとき（撮影・recon・削除・オートクレーブ・位置安定化）

**所要はその場で計算する。**下の実測値から出す。分からなければ「未実測」と書いて、
終わったときに実測を残す。推測の数字を入れない。

## 何を書くか — 必ず2件セット

| | Status | 期限 | 名前の形 |
|---|---|---|---|
| 装置占有 | `Remind` | start = 開始時刻 / **end = 見込み終了** | `【顕微鏡】grid 撮影中（Pos0＋Pos64–98 / 36 Pos）` |
| 次の作業 | `NextAction` | start = **見込み終了時刻** | `撮影終了 → recon 再開（C: 読み・E: 書き）` |

- Tag は `["実験"]` か `["QPI"]`
- 名前の頭に `【顕微鏡】` `【解析PC】` を付ける。Mac 側の秘書がこれで出所を見分ける
- 本文に、計算の根拠（何 Pos × 何分）と、そのとき決めた保存先・容量を1行ずつ書く

## 待ちが終わったら end を実測に直す

**ここが本体。**見込みで置いた `end` を、実際に終わった時刻に書き換える。
ずれた理由も本文に1行足す（「削除待ちで 1h40m 遅れた」など）。
これをやらないと、Mac 側は見込みのまま翌日を組んで、また同じずれ方をする。

## 触ってよい範囲（越えないこと）

`check.py`（配置の検査）は Obsidian Vault の中にあって Windows には無い。だから:

- **作ってよいのは装置占有1件と、その直後の1件だけ**
- **既存タスクの日付を動かさない。Done にしない。削除しない**
- 固定タスク（部活・jog・weight）には一切触らない
- 衝突（占有帯に別の実験が刺さっている等）の解決は Mac 側の朝の routine でやる。
  気づいたら本文に1行書いて残すだけにする

## 書き込み先

Tasks DB の `data_source_id` は `7fb3f2db-e277-4883-b686-b364b2be9df7`。
`notion-create-pages` の parent に必ずこれを指定する。

- 期限は展開プロパティ: `date:期限:start` / `date:期限:end` / `date:期限:is_datetime`（数値の 1）
- **時刻は必ず `+09:00` を明示する。**UTC で書かない
- Done にするときは `date:期限:end` を必ず入れる（例外なし）

MCP が繋がっていない場合は `NOTION_TOKEN` を `~/.secrets/.env` に置いて API を直接叩く
（Mac 側の `scheduling-harness/dump_tasks.py` と同じやり方）。

# 装置と解析の実測値

**推測で見積もらない。この表から計算する。**値が古くなったら測り直してここを直す。
...
**容量を空けたいときは GB ではなくファイル数を見る。**同じ HDD で 400 倍違った
（380 GB / 230万ファイル ＝ 9.7 時間 vs 282 GB / 2万ファイル ＝ 1.5 分）。

## ディスクの役割

| | 性質 | 役割 |
|---|---|---|
| C: | システムディスク＋HDD | raw hologram。本来ここに撮るべきではないが、他が空かないときは戻す |
| D: | `AquisitionData` | 過去セッション置き場。空きがあれば撮影先 |
| E: | NVMe | **位相再構成の出力** |
| F: | 外付け | バックアップ。`_robocopy_*_log.txt` で照合してから D:/E: を消す |

撮影・raw 読み・recon 出力を別のディスクに割り振れば I/O が競合しないので、
**recon を止めずに撮影と並走できる**。同じディスクに2つ乗せると撮影が落ちることがある
（2026-09-09 の Pos50 の image saving 失敗。watcher が5分ごとに 6000 個の点フォルダを
開いていたのが一番疑わしいが確定ではない）。**撮影中は撮影先ディスクに触らない。**
