# タスク・スケジュール・アラームのルーティングルール

## Google Calendar（アラーム・通知専用）

以下のキーワードが含まれる場合は **Google Calendar MCP** を使ってイベントを作成する：
- 「アラームを鳴らして」「アラームを入れて」
- 「通知して」「通知を入れて」
- 「リマインドして」「リマインダーを入れて」
- 「〇時に知らせて」「〇時に教えて」
...
Google Calendarイベントには必ずリマインダーを設定すること（デフォルト: 10分前）。

## ClickUp（タスク・予定管理）

以下のキーワードが含まれる場合は **`python scripts/clickup_helper.py add`** を使ってタスクを作成する：
- 「予定を入れて」「予定を追加して」
- 「タスクを入れて」「タスクを追加して」
- 「スケジュールして」「ToDo」
- 「やること」「やるべきこと」
