"""
watch_grid_then_recon_260819_b3.py

Batch 3 (final) of the 260819 grid session.
  b1  Pos0..Pos35       -> E:\260819\grid_ye_1          done, raw deleted
  b2  Pos0 + Pos36..70  -> E:\260819\grid_b2_pos36_70   done, raw deleted

  raw     D:\AquisitionData\Kitagishi\260819\grid_ye_3   Pos0 + Pos71..Pos102 (3993 points)
  output  E:\260819\grid_b3_pos71_102

Like b2, this batch carries its own Pos0 BG and therefore writes to its own
output dir. The raw on D: is deleted once the reconstruction verifies, so ALL
11 z planes are reconstructed (RECON_Z_INDICES = None).

Run:
    python watch_grid_then_recon_260819_b3.py
    python watch_grid_then_recon_260819_b3.py --dry-run    # patch + report only
    python watch_grid_then_recon_260819_b3.py --skip-wait  # acquisition already done
"""
import argparse
import re
import shutil
...
WATCH_DIR = Path(r"D:\AquisitionData\Kitagishi\260819\grid_ye_3")
WATCH_POS = [0] + list(range(71, 103))          # Pos0 (BG) + Pos71..Pos102

BATCHES = [
    {
        "name": "b3",
        "grid_dir": WATCH_DIR,
        "output_dir": Path(r"E:\260819\grid_b3_pos71_102"),
        "pos_list": WATCH_POS,
        "delete_raw": True,                      # frees D: (~590 GB)
    },
]
...
POS_SPLIT     = 51      # unchanged; Pos71..Pos102 fall on crop_after
...
# throw the rest away permanently. ~23 MB/point x 3993 points = ~92 GB.
...
LOG_PATH   = SESSION_DIR / "watch_grid_then_recon_b3.log"
ALERT_PATH = SESSION_DIR / "watch_grid_then_recon_b3.ALERT.txt"
# ============================================================
...
def wait_for_acquisition():
    ...
    last_label = f"Pos{max(WATCH_POS)}_x+{GRID_HALF}_y+{GRID_HALF}"
...
def main():
    ...
    log("=" * 70)
    log(f"watch_grid_then_recon_260819_b3  (POS_SPLIT={POS_SPLIT}, Z_INDEX={Z_INDEX})")
    log("=" * 70)
