"""
watch_grid_then_recon_260819_b2.py

Batch 2 of the 260819 grid session. Batch 1 (Pos0..Pos35, raw grid_ye_1) is
done: reconstructed to E:\260819\grid_ye_1 and its raw already deleted.

  raw     D:\AquisitionData\Kitagishi\260819\grid_ye_2   Pos0 + Pos36..Pos70 (4356 points)
  output  E:\260819\grid_b2_pos36_70

Batch 2 carries its own Pos0 BG, so it reconstructs against that and writes to
its own output dir -- writing into grid_ye_1 would overwrite batch 1's Pos0
output with a BG shot taken hours later.

As in batch 1, the raw on D: is deleted once the reconstruction verifies, so
ALL 11 z planes are reconstructed (RECON_Z_INDICES = None).

Run:
    python watch_grid_then_recon_260819_b2.py
    python watch_grid_then_recon_260819_b2.py --dry-run    # patch + report only
    python watch_grid_then_recon_260819_b2.py --skip-wait  # acquisition already done
"""
import argparse
import re
import shutil
...
WATCH_DIR = Path(r"D:\AquisitionData\Kitagishi\260819\grid_ye_2")
WATCH_POS = [0] + list(range(36, 71))           # Pos0 (BG) + Pos36..Pos70

BATCHES = [
    {
        "name": "b2",
        "grid_dir": WATCH_DIR,
        "output_dir": Path(r"E:\260819\grid_b2_pos36_70"),
        "pos_list": WATCH_POS,
...
def main():
    ...
    log("=" * 70)
    log(f"watch_grid_then_recon_260819_b2  (POS_SPLIT={POS_SPLIT}, Z_INDEX={Z_INDEX})")
    log("=" * 70)
