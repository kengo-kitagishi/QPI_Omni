"""
watch_grid_then_recon_260917_b2.py

Batch 2 of the 260917 grid session (batch 1 = Pos0..Pos40, raw already deleted).

  raw     E:\260917\grid_hologram_0p05_2          Pos0 (BG) + Pos41..Pos99 (60 Pos)
  output  D:\AquisitionData\Kitagishi\260917\grid_hologram_0p05_2

Separate output directory from batch 1: this batch has its own Pos0 BG, and
writing into grid_hologram_0p05_1 would overwrite batch 1's Pos0 output.

This batch straddles POS_SPLIT=52: Pos41..51 reconstruct on crop_before,
Pos52..99 on crop_after. batch_reconstruction_grid switches per Pos and builds
the Pos0 BG for both crops, so no special handling is needed here.

Waits until every point of EXPECTED_POS holds images and nothing new has been
written for IDLE_MINUTES, then reconstructs all 11 z planes, detects channels,
calibrates, verifies, and deletes the raw on E: once verification passes.

Run:
    python watch_grid_then_recon_260917_b2.py
    python watch_grid_then_recon_260917_b2.py --dry-run --skip-wait
    python watch_grid_then_recon_260917_b2.py --skip-wait
"""
import argparse
import re
import shutil
...
WATCH_DIR = Path(r"E:\260917\grid_hologram_0p05_2")
OUTPUT_DIR = Path(r"D:\AquisitionData\Kitagishi\260917\grid_hologram_0p05_2")
...
BATCHES = [
    {
        ...
        "delete_raw": True,                      # frees E: (~1.2 TB) after verification
    },
]
...
EXPECTED_POS  = [0] + list(range(41, 100))   # Pos0 (BG) + Pos41..Pos99
...
IDLE_MINUTES  = 10      # quiet time required after every EXPECTED_POS is complete
POLL_MINUTES  = 5       # each point folder is opened once (see _CONFIRMED)
MIN_FREE_GB_OUT = 200   # recon output ~2.6 GB/Pos -> ~156 GB for 60 Pos
MIN_FREE_GB_RAW = 60    # alert when E: gets this low during acquisition

# Beep on completion/failure: this run ends unattended and both the raw
# deletion and the next acquisition step wait on it.
BEEP_ENABLED = True

LOG_PATH   = SESSION_DIR / "watch_grid_then_recon_b2.log"
ALERT_PATH = SESSION_DIR / "watch_grid_then_recon_b2.ALERT.txt"
# ============================================================
...
def wait_for_acquisition():
    ...
    Completion: every Pos in EXPECTED_POS complete and no new point for IDLE_MINUTES.
    """
    ...
    while True:
        ...
        if n != last_n:
            log(f"  {n} points  ({len(complete)} complete Pos"
                ...
                + f")  idle={idle_min:.0f}min  {WATCH_DIR.anchor}free={c_free:.0f}GB")
            last_n = n
        ...
        beyond = sorted(pos for pos in by_pos if pos not in EXPECTED_POS)
        if beyond and not beyond_warned:
            alert(f"unexpected Pos on disk: {beyond}. Still waiting for the "
                  f"expected Pos to complete.")
            beyond_warned = True
        all_done = all(pos in complete for pos in EXPECTED_POS)
        if all_done and idle_min >= IDLE_MINUTES:
...
def main():
    ...
    log("=" * 70)
    log(f"watch_grid_then_recon_260917_b2  (POS_SPLIT={POS_SPLIT}, Z_INDEX={Z_INDEX})")
    log("=" * 70)
