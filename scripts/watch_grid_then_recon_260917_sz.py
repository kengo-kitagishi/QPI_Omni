...
Measured while it ran: 0.33 s/point = 0.7 min/Pos, 22.9 MB/point = 2.8 GB/Pos.

Waits until every point of Pos0..LAST_POS is on disk and nothing new has been written for
IDLE_MINUTES, then reconstructs, detects channels, calibrates and verifies. The raw is kept:
it is the reference for the timelapse, and at 277 GB for 100 Pos E: has room.

Run:
    python watch_grid_then_recon_260917_sz.py
    python watch_grid_then_recon_260917_sz.py --dry-run --skip-wait
"""
import argparse
import re
import shutil
...
WATCH_DIR = Path(r"E:\260917\grid_hologram_0p05_3")
OUTPUT_DIR = Path(r"D:\AquisitionData\Kitagishi\260917\grid_hologram_0p05_3")
...
BATCHES = [
    {
        ...
        "delete_raw": False,                     # raw stays on E:
    },
]

GRID_HALF     = 5       # 11 x 11 = 121 points per Pos
N_Z           = 1       # single-z grid: one plane per point
...
RECON_Z_INDICES = [0]   # the one plane; the raw is kept either way

IDLE_MINUTES  = 10      # quiet time required after Pos0..LAST_POS are all complete
POLL_MINUTES  = 5       # each point folder is opened once (see _CONFIRMED)
MIN_FREE_GB_OUT = 80    # recon output ~0.25 GB/Pos -> ~25 GB for 100 Pos
...
LOG_PATH   = SESSION_DIR / "watch_grid_then_recon_sz.log"
ALERT_PATH = SESSION_DIR / "watch_grid_then_recon_sz.ALERT.txt"
# ============================================================
...
def wait_for_acquisition():
    ...
    Completion: Pos0..LAST_POS all complete and no new point for IDLE_MINUTES.
    """
    ...
    while True:
        ...
        beyond = sorted(pos for pos in by_pos if pos > LAST_POS)
        if beyond and not beyond_warned:
            alert(f"acquisition went past Pos{LAST_POS}: {beyond} on disk. "
                  f"Still waiting for it to go idle.")
            beyond_warned = True
        all_done = all(pos in complete for pos in range(LAST_POS + 1))
