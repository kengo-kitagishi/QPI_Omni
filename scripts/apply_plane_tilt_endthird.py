...
Only the frames in FRAMES are written (a contact sheet needs one frame per channel), into a
sibling tree ``<sub>_plane/chNN/``. Nothing is overwritten.
"""
import argparse
import os
import re
...
ap.add_argument("--root", required=True, help="session root holding PosN/output_phase/channels/...")
ap.add_argument("--sub", default="crop_sub_rawraw/z000", help="channel dir relative to channels/")
ap.add_argument("--frames", type=int, nargs="+", required=True)
ap.add_argument("--pos", type=int, nargs="+", default=None, help="default: every Pos found")
ap.add_argument("--pos-split", type=int, default=52, help="Pos >= this fits the right third")
...
for pos, d in pos_dirs:
    ...
    for ch in sorted(x for x in os.listdir(base) if re.match(r"^ch\d+$", x)):
        ...
        n_ch += 1
        n_frames += wrote
print(f"{n_ch} channels, {n_frames} frames -> <{a.sub}{a.suffix}>  (plane on the "
      f"{'right' if a.pos_split == 0 else 'aperture-end'} third, fit_right by Pos >= {a.pos_split})")
