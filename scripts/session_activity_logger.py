...
def render_git_block(git_info: Dict[str, Any], runtime: Dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- Target OS: {runtime['os']}",
        f"- User: {runtime['user']}",
        f"- Token/config source: Git metadata (`{runtime['repo_root']}`).",
        "- Backup policy: state observation only. No file updates performed.",
        "",
        "### Background",
        "- Captured a Git snapshot to keep a periodic record of repository state in the work log.",
        "",
        "### Requirements (user requirements)",
        "- Objective: Record the current repository state in a reproducible form.",
        f"- Scope: branch/HEAD/status/latest commit of `{runtime['repo_root']}`.",
        "- Expected deliverable: A work-log block summarizing Git state with requirements, steps, and verification.",
        "- Acceptance criteria: branch/HEAD/latest commit/changed files are recorded consistently.",
        "- Constraints/assumptions: Only read-only git commands are used; no history-rewriting commands executed.",
        "- Unresolved items: none",
        "",
        "### Implementation policy",
        "- Adopted policy: Fix the state using three systems: `git rev-parse` / `git status --short` / `git show`.",
        "- Rationale: Covers history point, working-tree diff, and latest context with a minimal set of commands.",
        "- Execution order: Confirm HEAD -> confirm change status -> confirm latest commit -> consistency check.",
        "- Alternative and rejection reason: `git log --stat` alone misses uncommitted working-tree diffs, so rejected.",
        "- Risks and mitigations: Risk that state changes during execution is mitigated by running the commands consecutively within the same block and recording timestamps.",
        "",
        "### Implementation steps (Step 0, 1, 2... with commands)",
        "#### Step 0: Backup (optional)",
        "- Objective: Save a snapshot of the current notes for state comparison.",
        "```bash",
        f"cp \"{daily_file}\" \"{daily_file}.bak\"",
        "```",
        "- Expected result: `.bak` file is created.",
        "- Actual result: not executed (only collecting Git snapshot).",
        "",
        "#### Step 1: Verify branch and HEAD",
        "- Objective: Fix which history point is being observed.",
        "```bash",
        f"cd \"{runtime['repo_root']}\"",
        "git rev-parse --abbrev-ref HEAD",
        "git rev-parse HEAD",
        "```",
        "- Expected result: branch name and HEAD SHA are obtained.",
        f"- Actual result: branch={git_info['branch']}, head={git_info['head'][:12]}",
        "",
        "#### Step 2: Verify change status",
        "- Objective: Get whether the working tree has diffs.",
        "```bash",
        f"cd \"{runtime['repo_root']}\"",
        "git status --short",
        "```",
        "- Expected result: Uncommitted changes are obtained line by line.",
        f"- Actual result: changed_files={len(git_info['changed_files'])}",
        "",
        "#### Step 3: Verify latest commit summary",
        "- Objective: Record the most recent commit with timestamp.",
        "```bash",
        f"cd \"{runtime['repo_root']}\"",
        "git show -s --format=%h\\ %ci\\ %s HEAD",
        "```",
        "- Expected result: One line in `shortSHA timestamp subject` format.",
        f"- Actual result: {git_info['latest']}",
        "",
        "#### Step 4: Final verification checklist",
        "- Objective: Check whether any required information is missing.",
        "```bash",
        f"cd \"{runtime['repo_root']}\"",
        "git rev-parse --abbrev-ref HEAD",
        "git status --short",
        "git show -s --format=%h\\ %ci\\ %s HEAD",
        "```",
        "- Expected result: Consistent with the results of Step 1-3.",
        "- Actual result: pass (consistent).",
        "",
        "### Verification steps and results",
        ...
        "```",
        f"- Actual result: branch={git_info['branch']}, HEAD={git_info['head'][:12]}",
        "2. Verify change status and latest commit",
        "```bash",
        f"cd \"{runtime['repo_root']}\" && git status --short && git show -s --format=%h\\ %ci\\ %s HEAD",
        "```",
        f"- Actual result: latest_commit={git_info['latest']}, changed_files={len(git_info['changed_files'])}",
        "- Verdict: pass",
        f"- Evidence: repo=`{runtime['repo_root']}` / daily_note=`{daily_file}`",
        "",
        "### Changed files",
    ]
    ...
    if git_info["changed_files"]:
        for item in git_info["changed_files"][:30]:
            lines.append(f"- `{item}`: Observed by `git status --short`. Recorded for state tracking.")
        if len(git_info["changed_files"]) > 30:
            lines.append(f"- `... ({len(git_info['changed_files']) - 30} more)`: Additional entries omitted.")
    else:
        lines.append("- none")
    ...
    lines.extend([
        "",
        "### Known constraints and notes",
        "- This block is a point-in-time Git observation; it does not include conversation-derived requirement details.",
        "- `git status --short` is a snapshot value at execution time and may change with subsequent operations.",
        "",
        "### Reproduction steps on another PC",
        "1. Replace `repo_root` with the actual path on the target PC.",
        "2. Execute Steps 1-4 of the implementation steps in order.",
        "3. Check the verdict and evidence of the verification steps.",
    ])
