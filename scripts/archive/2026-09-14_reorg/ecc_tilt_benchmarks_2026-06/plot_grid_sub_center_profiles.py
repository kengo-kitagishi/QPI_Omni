...
---------------------------------
Apply ECC alignment to each channel crop and then pixel-wise subtract grid_ref_crops,
displaying center horizontal line profiles (row Y=center_y) overlaid for multiple TPs.

Processing:
  1. Obtain channel crop (crop_w x crop_h) from each TP phase image
  2. Subtract backsub offset and normalize to float
  3. to_uint8 -> ecc_align to obtain (tx, ty)
  4. Apply (-tx, -ty) warp to float crop (align sample to ref coordinates)
  5. Pixel-wise subtract grid_ref_crop
  6. Extract center row profile (Y=center_y, length crop_h)
  7. Plot all channels in 4 columns (color-coded by TP)
"""
...
# TP indices to display (0-origin)
...
def main():
    ...
    if not phase_paths:
        print(f"ERROR: Phase images not found: {PHASE_DIR}")
        sys.exit(1)
    ...
    # Pre-compute backsub'd float and uint8 for grid_ref
    cfg_dummy = {}
    ref_u8_list   = []
    ref_bs_list   = []
    for c in range(n_ch):
    ...
    for i_show, tp_idx in enumerate(pick_tps):
        path     = phase_paths[tp_idx + 1]   # +1: index 0 is the reference frame
        ...
        for c_idx, roi in enumerate(rois):
            ...
            # 3. Apply (-tx, -ty) warp to float crop (align sample to ref)
            M = np.float32([[1, 0, -tx], [0, 1, -ty]])
            aligned = cv2.warpAffine(crop_bs, M, (crop_h, crop_w),
                                     flags=cv2.INTER_LINEAR,
                                     borderMode=cv2.BORDER_REPLICATE)

            # 4. Pixel-wise subtract grid_ref
            diff = aligned - ref_bs_list[c_idx]

            # 4b. Slope+intercept correction: linear fit on left 1/3 (background region) -> subtract
            ...
            # 5. Center row profile
            center_y = crop_w // 2
            profiles[i_show][c_idx] = diff[center_y, :]   # (crop_h,) = 80 points
