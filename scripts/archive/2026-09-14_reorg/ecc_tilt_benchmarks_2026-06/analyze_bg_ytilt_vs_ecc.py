...
--------------------------
Verify per-frame correlation between the background Y-direction tilt
(wavefront Y-tilt) of reconstructed images and the ECC Y-direction shift.

Hypothesis:
  Per-frame wavefront linear tilt (Y direction) generates common
  ECC Y error across all channels -> is this the cause of inter-channel correlation (r=0.41)?

Processing:
  1. Compute ECC Y shift for all channels per frame -> take frame-mean
  2. In the same frame, extract background ROI (left side of image, cx=BG_CX, crop_h=BG_CROP_H px width),
     compute row mean -> create a 240-point Y profile
  3. Linear fit all 240 points with np.polyfit(..., 1) -> slope [rad/px]
...
# Background ROI (left side background region of the image)
BG_CX       = 100   # background cx (channels are at cx~360, so sufficiently left)
BG_CY       = 255   # image center
BG_CROP_W   = 240   # Y-direction size [px] (to observe tilt from end to end)
BG_CROP_H   = 30    # X-direction size [px] (thin strip)

# Y profile is in a pure background region -> linear fit on all points
...
def compute_y_tilt(phase_img: np.ndarray) -> float:
    """Polyfit all points of the background Y profile -> return slope [rad/px]."""
    crop = extract_rect_roi(phase_img, BG_CY, BG_CX, BG_CROP_W, BG_CROP_H)
    row_mean = crop.mean(axis=1).astype(np.float64)   # (BG_CROP_W,) Y profile
...
def main():
    ...
    # ---- ECC Y shift (all channels) ----
    # grid_ref_crops.tif = backsub-applied float32 -> to_uint8 for ECC reference
    ...
    ty_raw = np.full((n_ch, n_tp), np.nan)   # image Y direction [px]
    ...
    # (C) per-channel scatter (all ch x all TP scatter, relationship with background tilt)
    ax2 = fig.add_subplot(gs[2])
    # Broadcast tilt to each TP -> one-to-one correspondence with all ch
