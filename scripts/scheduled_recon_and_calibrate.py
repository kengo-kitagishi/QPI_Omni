...
GRID_DIR = Path(r"E:\260917\grid_hologram_0p05_1")
...
def _calibrate_one(args):
    ...
    cgp.GRID_DIR = str(OUTPUT_DIR if OUTPUT_DIR != GRID_DIR else GRID_DIR)
...
def main():
    out_root = OUTPUT_DIR if OUTPUT_DIR != GRID_DIR else GRID_DIR
    ...
    cal_skip = 0
    for label in labels:
        out_path = out_root / f"grid_calibration_{label}.json"
        if out_path.exists():
            cal_skip += 1
            continue
        rois_path = (out_root / f"{label}_x+0_y+0" / "output_phase"
