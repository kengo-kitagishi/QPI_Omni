"""
watch_grid_then_recon_260810.py

Driver for the 260810 grid session, which ended up split across two batches
because C: filled up mid-acquisition.

  batch 1  Pos0..Pos71            raw on C:\\260810\\grid_ye_1            (8712 points)
  batch 2  Pos0 + Pos72..Pos102   raw on D:\\...\\260810\\grid_ye_1        (3872 points)

Both batches carry their own Pos0, so each reconstructs against its own BG.
Reconstruction output goes to E: for both — D: is the batch-2 acquisition
target and must not be written to by the pipeline.

What this does:
  1. Wait until the batch-2 acquisition finishes (all points present AND no new
     files for IDLE_MINUTES).
  2. For each batch: patch the pipeline config, run
     scheduled_recon_and_calibrate.py (recon -> channel detect -> calibration),
     verify the output.
  3. Delete the batch-1 raw on C: once its output verifies, to free the volume.
     Batch-2 raw on D: is never deleted.

Run:
    python watch_grid_then_recon_260810.py
    python watch_grid_then_recon_260810.py --dry-run    # patch + report only
    python watch_grid_then_recon_260810.py --skip-wait  # acquisition already done
    python watch_grid_then_recon_260810.py --only b2    # single batch
"""
import argparse
import re
import shutil
...
# Configuration
# ============================================================
SESSION_DIR = Path(r"C:\260810")

# The acquisition this script waits for before doing anything.
WATCH_DIR = Path(r"D:\AquisitionData\Kitagishi\260810\grid_ye_1")
WATCH_POS = [0] + list(range(72, 103))          # Pos0 + Pos72..Pos102

BATCHES = [
    {
        "name": "b1",
        "grid_dir": Path(r"C:\260810\grid_ye_1"),
        "output_dir": Path(r"E:\260810\grid_b1_pos0_71"),
        "pos_list": list(range(0, 72)),          # Pos0..Pos71
        "delete_raw": True,                      # frees C:
    },
    {
        "name": "b2",
        "grid_dir": WATCH_DIR,
        "output_dir": Path(r"E:\260810\grid_b2_pos72_102"),
        "pos_list": WATCH_POS,
        "delete_raw": False,                     # D: raw is the only copy
    },
]
...
# can be re-reconstructed at other z later; batch 1's raw is already deleted.
RECON_Z_INDICES = [Z_INDEX]

IDLE_MINUTES  = 15      # acquisition considered finished after this much quiet
POLL_MINUTES  = 5
MIN_FREE_GB_E = 350     # recon output for both batches is ~290 GB

# Audible alerts are off: alerts go to the log and to ALERT_PATH instead.
BEEP_ENABLED = False
...
def main():
    ...
    ap.add_argument("--skip-wait", action="store_true",
                    help="assume the batch-2 acquisition is already finished")
    ap.add_argument("--only", choices=[b["name"] for b in BATCHES],
                    help="run a single batch")
    args = ap.parse_args()

    log("=" * 70)
    log(f"watch_grid_then_recon_260810  (POS_SPLIT={POS_SPLIT}, Z_INDEX={Z_INDEX})")
    log("=" * 70)
