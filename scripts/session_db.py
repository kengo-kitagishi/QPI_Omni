...
SQLite cache/index layer for Claude Code sessions and figure events.

- sessions        : Session metadata (metrics, state flags, design notes)
- session_files   : Files edited within sessions (operation, old/new preview)
- tool_calls      : All tool invocations (name, input, result, error)
- bash_commands   : Bash commands (command, description, output, error)
- figure_events   : figure-hub inbox figure events (including enrichment state)
- processing_runs : Processing run history (processed/skipped/error counts)

DB path: .figure_history/sessions.db
Auto-migrates from existing JSON state files on first startup.

CLI:
  python3 scripts/session_db.py --stats      Show statistics
  python3 scripts/session_db.py --sessions   List sessions (last 20)
  python3 scripts/session_db.py --week 2026-W10  Weekly summary
"""
...
def open_db(db_path: Path = DB_PATH) -> sqlite3.Connection:
    """Open the DB, initialize schema, and return the connection. Caller must close()."""
...
def get_conn(db_path: Path = DB_PATH) -> Generator[sqlite3.Connection, None, None]:
    """Context manager version. Caller is responsible for commit (for batch processing in loops)."""
...
# ────────────────────────────────────────────
# Migration from JSON state (first run only)
# ────────────────────────────────────────────

def _migrate_from_json(conn: sqlite3.Connection) -> None:
    """Migrate minimal data from existing JSON state files (idempotent)."""
    migrated_sessions = 0
    migrated_figures = 0
    ...
    if migrated_sessions > 0 or migrated_figures > 0:
        print(f"  DB initialized: migrated {migrated_sessions} sessions, {migrated_figures} figure events from JSON state")
...
def get_session_mtime(conn: sqlite3.Connection, session_id: str) -> Optional[float]:
    """Return the last processed mtime recorded in DB. None = not yet processed."""
    row = conn.execute(
...
def delete_session_parts(conn: sqlite3.Connection, session_id: str) -> int:
    """Delete rows with synthesized IDs generated by date splitting. The original session_id row is preserved.
    session_files, tool_calls, bash_commands are auto-deleted via CASCADE.
...
def is_figure_processed(conn: sqlite3.Connection, unique_key: str) -> bool:
    """Return whether unique_key exists in DB and is not a migration record (actually processed)."""
    row = conn.execute(
...
def query_session_code_changes(conn: sqlite3.Connection, session_id: str) -> list[dict]:
    """Return files and contents for Edit/Write operations (for code change details)."""
    rows = conn.execute(
...
def query_week_file_stats(conn: sqlite3.Connection, monday: str, sunday: str) -> list[dict]:
    """Return file change statistics for the week."""
    rows = conn.execute(
        """SELECT sf.file_path,
...
def query_week_stats(conn: sqlite3.Connection, monday: str, sunday: str) -> dict:
    """Return aggregate statistics for the week."""
    row = conn.execute(
        """SELECT
...
def _print_stats(conn: sqlite3.Connection) -> None:
    print("=== sessions.db statistics ===")
    for label, sql in [
        ("Total sessions", "SELECT COUNT(*) FROM sessions"),
        ("  Pending reprocess", "SELECT COUNT(*) FROM sessions WHERE needs_reprocess = 1"),
        ("Total tool calls", "SELECT COUNT(*) FROM tool_calls"),
        ("Total bash commands", "SELECT COUNT(*) FROM bash_commands"),
        ("  Errors", "SELECT COUNT(*) FROM bash_commands WHERE is_error = 1"),
        ("Total figure events", "SELECT COUNT(*) FROM figure_events WHERE json_path NOT LIKE '(migrated:%'"),
        ("  Enriched", "SELECT COUNT(*) FROM figure_events WHERE enriched = 1"),
        ("Processing run logs", "SELECT COUNT(*) FROM processing_runs"),
    ]:
        n = conn.execute(sql).fetchone()[0]
        print(f"  {label}: {n}")

    print("\n--- Last 5 processing_runs ---")
    rows = conn.execute(
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="sessions.db management tool")
    parser.add_argument("--stats", action="store_true", help="Show statistics")
    parser.add_argument("--sessions", action="store_true", help="List sessions (last 20)")
    parser.add_argument("--week", help="Weekly summary (e.g. 2026-W10)")
