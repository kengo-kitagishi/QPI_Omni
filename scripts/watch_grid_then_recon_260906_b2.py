...
Batch 2 of the 260906 grid session (batch 1 = Pos0..Pos35, raw already deleted).

  raw     D:\AquisitionData\Kitagishi\260906\grid_YE_0p05_hologram_Pos36_57_1
...
Reconstructed with the CORRECTED off-axis center (468,1665). Batch 1 was built
with the conjugate order and is being repaired by negate_phase_tree.py; this
batch needs no such fix.

The raw is deleted once the output verifies, so all 11 z planes are reconstructed.

Run:
    python watch_grid_then_recon_260906_b2.py
    python watch_grid_then_recon_260906_b2.py --dry-run
    python watch_grid_then_recon_260906_b2.py --skip-wait
"""
import argparse
import re
import shutil
...
# Configuration
# ============================================================
SESSION_DIR = Path(r"C:\260906")

# The acquisition this script waits for before doing anything.
WATCH_DIR = Path(r"D:\AquisitionData\Kitagishi\260906\grid_YE_0p05_hologram_Pos36_57_1")
WATCH_POS = [0] + list(range(36, 58))           # Pos0 (BG) + Pos36..Pos57
...
BATCHES = [
    {
        ...
        "pos_list": WATCH_POS,
        "delete_raw": True,                      # frees D: (~420 GB) after verification
    },
]
...
POLL_MINUTES  = 5
MIN_FREE_GB_OUT = 150   # recon output is ~100 GB (23 MB/point x 4356)
...
def main():
    ...
    log("=" * 70)
    log(f"watch_grid_then_recon_260906_b2  (POS_SPLIT={POS_SPLIT}, Z_INDEX={Z_INDEX})")
    log("=" * 70)
