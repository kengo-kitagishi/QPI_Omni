# %%
"""
新フォルダ構造対応のアライメント・引き算統合スクリプト（Raw画像版）
...
def find_matching_pos_folders(wo0_base, wo2_base, pos_start=None, pos_end=None):
    ...
    for pos_name in wo0_pos_folders:
        ...
        # 両方の output_phase と output_phase_raw フォルダが存在するか確認
        wo0_phase_dir = os.path.join(wo0_pos_path, "output_phase")
        wo2_phase_dir = os.path.join(wo2_pos_path, "output_phase")
        wo0_phase_raw_dir = os.path.join(wo0_pos_path, "output_phase_raw")
        wo2_phase_raw_dir = os.path.join(wo2_pos_path, "output_phase_raw")

        if (os.path.exists(wo0_phase_dir) and os.path.exists(wo2_phase_dir) and
            os.path.exists(wo0_phase_raw_dir) and os.path.exists(wo2_phase_raw_dir)):
...
def process_pos_complete(pos_name, wo0_phase_dir, wo2_phase_dir,
                         reference_number=None, method='ecc',
                         save_png=True, vmin=-0.1, vmax=1.7, cmap='RdBu_r',
                         png_dpi=150, png_sample_interval=1, pos_split=44):
    """
    ...
    pos_name : str
        Posフォルダ名（例: "Pos1"）
    wo0_phase_dir : str
        wo_0の位相画像ディレクトリ（output_phase）
    wo2_phase_dir : str
        wo_2の位相画像ディレクトリ（output_phase）
    reference_number : int or None
        基準画像の番号（None: 中央の画像を自動選択）
    method : str
    ...
    # reference_numberがNoneの場合、中央の画像番号を使用
    if reference_number is None:
        middle_index = len(wo0_files) // 2
        middle_file = wo0_files[middle_index]
        reference_number = get_image_number(middle_file)
        if reference_number is None:
            reference_number = 0
        print(f"  自動選択: 中央の画像（{middle_index + 1}/{len(wo0_files)}枚目、番号{reference_number}）を基準とします")
    ...
    # ===== ステップ2: アライメント適用 + 引き算（生の位相画像を使用） =====
    print(f"\n[2/3] アライメント適用 + 引き算処理中（生の位相画像を使用）...")

    # 出力ディレクトリ作成
    aligned_dir = os.path.join(wo2_phase_dir, "aligned")
    aligned_raw_dir = os.path.join(wo2_phase_dir.replace("output_phase", "output_phase_raw"), "aligned_raw")
    subtracted_dir = os.path.join(wo2_phase_dir, "subtracted_raw")
    os.makedirs(aligned_dir, exist_ok=True)
    os.makedirs(aligned_raw_dir, exist_ok=True)
    os.makedirs(subtracted_dir, exist_ok=True)

    if save_png:
        colored_dir = os.path.join(wo2_phase_dir, "subtracted_raw_colored")
    ...
    # rawディレクトリのパス
    wo0_raw_dir = wo0_phase_dir.replace("output_phase", "output_phase_raw")
    wo2_raw_dir = wo2_phase_dir.replace("output_phase", "output_phase_raw")
    ...
    for wo2_filename in tqdm(wo2_files, desc=f"    {pos_name}"):
        try:
            ...
            # ===== 生の位相画像を読み込み =====
            # wo_2の生画像
            wo2_raw_path = os.path.join(wo2_raw_dir, wo2_filename)
            if not os.path.exists(wo2_raw_path):
                if skipped_count < 3:
                    print(f"\n    ⚠️ 警告: wo_2の生画像が見つかりません: {wo2_filename}")
                skipped_count += 1
                continue
            wo2_raw_img = load_tif_image(wo2_raw_path)

            # アライメント適用（生画像に）
            h, w = wo2_raw_img.shape
            aligned_raw_img = cv2.warpAffine(
                wo2_raw_img.astype(np.float32),
                warp_matrix,
                (w, h),
            ...
            # アライメント済み生画像を保存
            aligned_raw_path = os.path.join(aligned_raw_dir, wo2_filename)
            io.imsave(aligned_raw_path, aligned_raw_img.astype(np.float32))

            # wo_0の生画像を読み込み
            wo0_raw_path = os.path.join(wo0_raw_dir, wo0_filename)
            if not os.path.exists(wo0_raw_path):
                if skipped_count < 3:
                    print(f"\n    ⚠️ 警告: wo_0の生画像が見つかりません: {wo0_filename}")
                skipped_count += 1
                continue
            wo0_raw_img = load_tif_image(wo0_raw_path)

            # サイズチェック
            if aligned_raw_img.shape != wo0_raw_img.shape:
                print(f"\n    ⚠️ 警告: 画像サイズが一致しません: {wo2_filename}")
                skipped_count += 1
                continue

            # 引き算: 生画像同士
            subtracted = aligned_raw_img - wo0_raw_img
            ...
            wo2_nobg_path = os.path.join(wo2_phase_dir, wo2_filename)
            wo2_nobg_img = load_tif_image(wo2_nobg_path)
            aligned_nobg_img = cv2.warpAffine(
                wo2_nobg_img.astype(np.float32),
                warp_matrix,
                (w, h),
                flags=cv2.INTER_LINEAR + cv2.WARP_INVERSE_MAP
            ).astype(np.float64)
    ...
    json_path = os.path.join(wo2_phase_dir, "alignment_transform_raw.json")
...
def main():
    ...
    print("="*80)
    print("新フォルダ構造対応のアライメント・引き算統合処理（Raw画像版）")
    print("アライメント: 背景引き算済み画像 / 引き算: 生の位相画像")
    print("="*80)

    # ベースディレクトリ設定
    WO0_BASE = r"e:\Acuisition\kitagishi\260212\wo\2_2"
    WO2_BASE = r"e:\Acuisition\kitagishi\260212\wo\2_1"
    ...
    for pos_name, wo0_phase_dir, wo2_phase_dir in pos_pairs:
        result = process_pos_complete(
            ...
            reference_number=None,  # None: 中央の画像を自動選択
            method='ecc',
            save_png=SAVE_PNG,
            vmin=VMIN,
            vmax=VMAX,
